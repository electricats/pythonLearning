...
data = finallist['areaTree'][0]['children']
provinceList = []
cityList = []

for i in data:
    provinceList.append(i['name'])
    cityList.append(i['children'])

userinput = input('请输入搜索的省份')
if userinput in provinceList:
    index = provinceList.index(userinput)
    for city in cityList[index]:
        print(city['name']+" 今日确诊数： "+ str(city['today']['confirm'])+" 总确诊人数： "+str(city['total']['confirm']))
